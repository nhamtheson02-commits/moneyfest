  ...
  return (
    <PageShell>
      <section className="mx-auto max-w-md px-4 py-14">
        ...
        <p className="mf-muted mt-3 text-sm">Sau khi đăng nhập, bạn sẽ được chuyển về đúng nội dung cần xem.</p>
        <div className="mt-6">
          <AuthForm action={loginAction} submitLabel="Đăng nhập" callbackUrl={callbackUrl}>
            <AuthField label="Email" name="email" type="email" autoComplete="email" />
            <AuthField label="Mật khẩu" name="password" type="password" autoComplete="current-password" />
          </AuthForm>
        </div>
        <div className="mt-4 flex justify-between text-sm font-semibold">
          <Link href={`/auth/register?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-[var(--mf-midnight)] hover:text-[var(--mf-gold)]">Tạo tài khoản</Link>
